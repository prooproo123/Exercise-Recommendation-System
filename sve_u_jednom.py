...
def run_all(path_to_dir, dataset_name, sep='\t'):
    ...
    print("TVC RADI")
    # kt
    params = kt.main(dataset_name, from_variable=True, variable_train=train_variable,
                     variable_valid=valid_variable)  # za sada se train i valid citaju iz csv-a, trebalo bi skuziti kako ih dobiti ko datafreameove iz skripte za stvaranje
    ...
    stu_only_ids = [el[0] for el in stu[0] if el[
        1] == 1]  # ako je netko krivo rijesio zadatak on bi i dalje trebao ostati u poolu mogucih zadataka
    ...
    recommendation = rs.run_rs(stu, cands, params, exercise_concepts_mapping, exercises_id_converter, no_exercises,
                               no_concepts)
