...
def main():
    ...
    with open('/home/zvonimir/Exercise-Recommendation-System/data/skill_builder/stand_ex_ind_con_ind.csv', 'rb') as f:
        a = pickle.load(f)

    data = DATA_LOADER(args.memory_size, 4, args.seq_len)
    data_directory = os.path.join(args.data_dir, args.dataset)

    with tf.Session(config=run_config) as sess:
        dkvmn = Model(args, sess, name='DKVMN')
        # dkvmn.getParam()
        with open('data/skill_builder/chunk_temp.pkl', 'rb') as f:
            temp = pickle.load(f)
