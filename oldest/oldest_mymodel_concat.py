...
from oldest import oldest_operations
from oldest.oldest_memory import DKVMN
...
class Model():
    ...
    def create_model(self):
        ...
        self.kg = tf.placeholder(tf.int32, [self.args.batch_size, self.args.seq_len, 4], name='knowledge_tag')
        self.kg_hot = tf.placeholder(tf.float32, [self.args.batch_size, self.args.seq_len, 123], name='knowledge_hot')
        ...
        for i in range(self.args.seq_len):
            ...
            summary_vector = tf.tanh(
                oldest_operations.linear(mastery_level_prior_difficulty, self.args.final_fc_dim, name='Summary_Vector',
                                         reuse=reuse_flag))
            # p_t
            pred_logits = oldest_operations.linear(summary_vector, 1, name='Prediction', reuse=reuse_flag)
    ...
    def train(self, train_q_data, train_qa_data, valid_q_data, valid_qa_data, train_kg_data,
              valid_kg_data, train_kgnum_data, valid_kgnum_data, traintime, validtime, trainguan, validguan, traindiff,
              validdiff):
        """
        ...
        for epoch in range(0, self.args.num_epochs):
            ...
            for steps in range(training_step):
                ...
                feed_dict = {self.kg: kg_batch_seq, self.q_data: q_batch_seq, self.qa_data: qa_batch_seq,
                             self.target: target_batch,
                             self.kg_hot: kghot_batch_seq, self.lr: self.args.initial_lr, self.timebin: time_batch_seq,
