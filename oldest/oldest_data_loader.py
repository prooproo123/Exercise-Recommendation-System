...
class DATA_LOADER():
    ...
    def load_dataes(self, q_data, qa_data, kg_data, kg_num, time, guan, diff):
        """
All resulting arrays are 2 dimensional, and len(var) might be the same for all of them. Therefore the shape is (len(var),seq_len=150).
They are being filled row by row with info from dataset, and the rest, up until seq_len, is zero.
