...
def main(dataset, path=None, from_variable=False, variable_train=None, variable_valid=None):
    print("START OF KT MAIN")
    ...
    with tf.Session(config=run_config) as sess:
        dkvmn = Model(memory_size=memory_size, batch_size=batch_size, seq_len=seq_len, n_questions=n_questions,
                      memory_key_state_dim=memory_key_state_dim,
                      memory_value_state_dim=memory_value_state_dim, final_fc_dim=final_fc_dim, momentum=momentum,
                      maxgradnorm=max_grad_norm,
                      show=show, init_from=init_from, checkpoint_dir=checkpoint_dir, log_dir=log_dir, dataset=dataset,
        ...
        if train:
            ...
            if from_variable:
                print("START OF LOAD2")
                train_q_data, train_qa_data = data.load_data2(variable_train)
                valid_q_data, valid_qa_data = data.load_data2(variable_valid)
            else:
